	  ...
	  <el-form :inline="true" :model="formInline" class="user-search">
        ...
        <ul>
          ...
          <li class="header" style="text-align: center">
		  	<div class='pro-img'style="margin-left: 110px"> Resume number</div>
    <div class='pro-img'style="margin-left: 130px"> name</div>

    <div class='pro-img'style="margin-left: 150px"> phone</div>
    <div class='pro-img' style="margin-left: 150px"> specialized</div>
          ...
          <li class="product-list" v-for="item in jianlixinxi" :key="item.id" style="text-align: center;display: block">
            <router-link :to="`/jianlixinxidetail/`+item.id" >
				<div class='pro-img' style="width: 180px">{{item.jianlibianhao}}</div>
    <div class='pro-img'style="width: 180px">{{item.xingming}}</div>

    <div class='pro-img' style="width: 180px">{{item.shouji}}</div>
    <div class='pro-img'style="width: 180px">{{item.zhuanye}}</div>
...
<script>
  export default {
    ...
    methods: {
	  ...
	  search() {
        this.getjianlixinxi(this.formInline)
      },
      // 向后端请求分类list数据
      getjianlixinxi(parameter) {
        this.$axios
               .post("/api/front/jianlixinxi", {...parameter})
                .then(res => {
                  debugger
                  this.jianlixinxi = res.data.data.list;
