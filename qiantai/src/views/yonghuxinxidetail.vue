<template>
  <div id="details">
    ...
    <div class="page-header">
      <div class="title" style="font-size: 14px;">
        <p>Home>> A list of job seekers >> Job seeker information is detailed </p>
      </div>
    </div>
    <div class="main">
		<el-card  shadow="hover" style="width: 560px; float:left">
			<div class="block">
				<div>
					<img style="height:500px;width: 500px" :src="`/api/`+yonghuxinxidetail.touxiang" />
				</div>
			</div>
		</el-card>
      <div class="content">
  <table width="100%" border="0"  align="center" cellpadding="3" cellspacing="1" class="newsline">
								<tr>
        <td width='22%' height=44>Username：</td><td width='39%' >{{yonghuxinxidetail.yonghuming}}</td></tr><tr>
         <td width='22%' height=44>name：</td><td width='39%' >{{yonghuxinxidetail.xingming}}</td></tr><tr>
         <td width='22%' height=44>gender：</td><td width='39%'><div v-if="yonghuxinxidetail.xingbie==1">man</div><div v-else>woman</div></td></tr><tr>
         <td width='22%' height=44>Password：</td><td width='39%'>******</td></tr><tr>
         <td width='22%' height=44>phone：</td><td width='39%' >{{yonghuxinxidetail.shouji}}</td></tr><tr>
         <td width='22%' height=44>identity card：</td><td width='39%'>******</td></tr><tr>

         <td width='22%' height=44>comment：</td><td width='39%' >{{yonghuxinxidetail.beizhu}}</td></tr>
  ...
  export default {
    data() {
      return {
        ...
        yonghuxinxiID:'',
        yonghuxinxidetail: "", // 商品Detailed信息
    ...
    created() {
      this.yonghuxinxiID= this.$route.params.id;
    ...
    methods: {
		handleEdit: function () {
                ...
                this.editForm.biao="yonghuxinxi";
                this.editForm.shujuid=this.yonghuxinxiID;
                this.userInfo = JSON.parse(localStorage.getItem("user"));
                this.editForm.pinglunren= localStorage.getItem("username");
            },
            getpinglun() {
                this.$axios
                    .post("/api/front/pinglun", {biao:"yonghuxinxi",shujuid:this.yonghuxinxiID})
                    .then(res => {
      ...
      getDetails(val) {
        this.$axios
                .get("/api/front/getyonghuxinxidetail/"+val,)
                .then(res => {
                  debugger
                  this.yonghuxinxidetail = res.data.data;
                })
                .catch(err => {
                  return Promise.reject(err);
                });
        ...
        this.$axios
                .post("/api/front/addCollect", {
                  ...
                  shangpinxinxiID: this.yonghuxinxiID
                })
                .then(res => {
                  if (res.data.code == "0") {
