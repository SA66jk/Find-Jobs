<template>
  <div id="qiyexinxilisttp" class="myList">
    <ul>
      <li v-for="item in list" :key="item.id">
        <el-popover placement="top">
          <p>Are you sure to delete it？</p>
          <div style="text-align: right; margin: 10px 0 0">
            <el-button type="primary" size="mini" @click="deleteCollect(item.id)">sure</el-button>
          </div>
          <i class="el-icon-close delete" slot="reference" v-show="isDelete"></i>
        </el-popover>
		<router-link :to="`/qiyexinxidetail/`+item.id">
          <img :src="`/api/`+item.qiyezhaopian" alt />
          <h2>{{item.qiyemingcheng}}</h2>
        </router-link>
      </li>
