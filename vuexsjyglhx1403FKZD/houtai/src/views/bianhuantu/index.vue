<template>
	<div>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column sortable prop="mingcheng" label="name"></el-table-column>
			<el-table-column align='center' prop='tupian' label='format' width='820'><template slot-scope='scope'><el-image :src='scope.row.tupian' style='height:70px;width: 400px;'  v-if="scope.row.tupian"/><img src="../../../static/images/guanli.jpg" style='height:70px' v-else/></template></el-table-column>



			<el-table-column align="center" label="Operate" min-width="100">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="danger" @click="deleteBianhuantu(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan' ">Delete</el-button>
		...
		<el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
			<el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
				<el-form-item  label="name" prop="mingcheng"><el-input size="small" v-model="editForm.mingcheng" auto-complete="off" placeholder="Please enter a name" style='width:50%'></el-input></el-form-item>
...
export default {
	...
	data() {
		return {
			...
			rules: {
				mingcheng: [{ required: true, message: 'Please enter a name', trigger: 'blur' },
				],
				tupian: [{ required: true, message: 'Please enter format', trigger: 'blur' },
				],
	...
	methods: {
		...
		handleEdit: function (index, row) {
			...
			if (row != undefined && row != 'undefined') {
				this.title = 'Modify the carousel'
				this.editForm = row
			} else {
				this.title = 'Add Carousel'
				this.editForm = {}
					...
					if(this.editForm.id){
						bianhuantuEdit(this.editForm).then(res => {
							...
							if (res.code == '0') {
								...
								this.$message({
									type: 'success',
									message: 'The Illusion Diagram has been modified！'
								})
							} else {
								this.$message({
									type: 'info',
						...
						bianhuantuSave(this.editForm).then(res => {
							...
							if (res.code == '0') {
								...
								this.$message({
									type: 'success',
									message: 'The change map was added successfully！'
								})
							} else {
								this.$message({
									type: 'info',
