<template>
	<div>
		<el-form :inline="true" :model="formInline" class="user-search">
			<el-form-item label=""><el-input size="small" v-model="formInline.jianlibianhao" placeholder="Enter your resume number"></el-input></el-form-item>
			<el-form-item label=""><el-input size="small" v-model="formInline.xingming" placeholder="Enter a name"></el-input></el-form-item>
			<el-form-item label=""><el-input size="small" v-model="formInline.shouji" placeholder="Enter your phone"></el-input></el-form-item>
			
			<el-form-item>
				<el-button size="small" type="warning" icon="el-icon-search" @click="search">Search</el-button>
				<el-button size="small" type="primary" icon="el-icon-plus" @click="handleEdit()" v-if="userInfo.roles[0] == 'yonghu'">Add</el-button>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" sortable prop="xingbie" label="gender" ><template slot-scope="scope"><el-switch v-model="scope.row.xingbie ? true : false" active-color="#13ce66" active-text="man" inactive-text="woman" inactive-color="#ff4949" @change="editxingbie(scope.$index, scope.row)"></el-switch></template></el-table-column><el-table-column sortable prop="shouji" label="phone"></el-table-column>
			<el-table-column sortable prop="zhuanye" label="specialized"></el-table-column>
			<el-table-column align='center' prop='zhaopian' label='Photo' width='120'><template slot-scope='scope'><el-image :src='scope.row.zhaopian' style='height:70px'  v-if="scope.row.zhaopian"/><img src="../../../static/images/guanli.jpg" style='height:70px' v-else/></template></el-table-column>
			<el-table-column sortable prop="yonghuming" label="Username"></el-table-column>
			<el-table-column align='center' sortable prop='wenjian' label='file' width='120'><template slot-scope='scope'><el-button @click="download(scope.row)">Download</el-button></template></el-table-column>
			...
			<el-table-column align="center" label="Operate" min-width="160">
				<template slot-scope="scope">
					...
					<!--liangbuan-->
					<el-button size="mini" type="success" @click="handleEdit(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'yonghu'">Edit</el-button>
					<el-button size="mini" type="danger" @click="deleteJianlixinxi(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'yonghu'">Delete</el-button>
		...
		<el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
			<el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
				<el-form-item  label="Resume number" prop="jianlibianhao"><el-input size="small" v-model="editForm.jianlibianhao" auto-complete="off" placeholder="Please enter your CV number" style='width:50%'></el-input></el-form-item>
		...
		<el-form-item  label="specialized" prop="zhuanye"><el-input size="small" v-model="editForm.zhuanye" auto-complete="off" placeholder="Please enter a major" style='width:100%'></el-input></el-form-item>
		<el-form-item  label='Photo' prop='zhaopian'><el-upload class='avatar-uploader' action='http://localhost:9999/files/upload' :headers='headers' :show-file-list='false' :on-success='handleZhaopianSuccess' :before-upload='beforeZhaopianUpload'><img v-if='editForm.zhaopian' :src='editForm.zhaopian' class='avatar'><i v-else class='el-icon-plus avatar-uploader-icon'></i></el-upload></el-form-item>
		<el-form-item  label="Biography" prop="gerenjianjie"><el-input type="textarea" size="small" v-model="editForm.gerenjianjie" auto-complete="off" placeholder="Please enter a bio" style='width:100%'></el-input></el-form-item>
		...
		<el-form-item  label='file' prop='wenjian'><el-upload class='avatar-uploader' action='http://localhost:9999/files/uploadwenjian' :headers='headers' :show-file-list='false' :on-success='handleWenjianSuccess' ><el-button>upload</el-button></el-upload></el-form-item>
...
import { jianlixinxiList, jianlixinxiSave, jianlixinxiDelete,jianlixinxiEdit,jianlixinxiDeleteList } from '@/api/jianlixinxi/jianlixinxiApi';
	...
	methods: {
		...
		handleShenheList(){
				...
				if(jianlixinxis.length == 0){
					...
					this.$confirm('Are you sure you want to review in bulk?', '信息', {
		...
		handleDetail(index, row) {const w = window.open("about:blank");w.location.href = 'http://localhost:8080/#/jianlixinxidetail/'+row.id;},
		//liangbuedit
		// Edit、增加页面Save方法
		submitForm(editData) {
		...
		deleteJianlixinxi(index, row) {
			this.$confirm('Are you sure you want to delete?', '信息', {
				confirmButtonText: 'sure',
				cancelButtonText: 'Cancel',
				type: 'warning'
			}).then(() => {
		...
		handleDeleteList(){
			...
			if(jianlixinxis.length == 0){
				...
				this.$confirm('Are you sure you want to delete in bulk?', '信息', {
					confirmButtonText: 'sure',
					cancelButtonText: 'Cancel',
					type: 'warning'
				}).then(() => {
