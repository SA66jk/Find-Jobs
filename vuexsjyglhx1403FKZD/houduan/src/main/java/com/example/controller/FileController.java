...
public class FileController {
    ...
    public Result upload(MultipartFile file, HttpServletRequest request) throws IOException {
        ...
        if (addInfo != null) {
            ...
            return Result.error("4001", "upload失败");
        }
    }
    ...
    public void download(@PathVariable String id, HttpServletResponse response) throws IOException {
        if ("null".equals(id)) {
            throw new CustomException("1001", "您未uploadfile");
        }
        FileInfo fileInfo = fileInfoService.findById(Long.parseLong(id));
        if (fileInfo == null) {
            throw new CustomException("1001", "未查询到该file");
        }
    ...
    public Result<FileInfo> getById(@PathVariable String id) {
        FileInfo fileInfo = fileInfoService.findById(Long.parseLong(id));
        if (fileInfo == null) {
            throw new CustomException("1001", "数据库未查到此file");
        }
        try {
            FileUtil.readBytes(BASE_PATH + fileInfo.getFileName());
        } catch (Exception e) {
            throw new CustomException("1001", "此file已被您Delete");
        }
