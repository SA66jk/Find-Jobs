...
// 判断路由 meta.roles 中是否包含当前loginJob Seekers权限字段
...
// 缓存多级嵌套数组处理后的一维数组(tagsView、菜单Search中使用：未过滤隐藏的(isHide))
export function setCacheTagsViewRoutes(arr) {
	// 先处理有权限的路由，否则 tagsView、菜单Search中无权限的路由也将显示
	let rolesRoutes = setFilterMenuFun(arr, store.state.userInfos.userInfos.roles);
	// Add到 vuex setTagsViewRoutes 中
...
// 处理后端return的 `component` 路径，拼装实现懒加载
...
// Add路由，模拟数据与方法，可自行进行修改 admin
// Add动态路由，`{ path: '*', redirect: '/404' }` 防止页面刷新，静态路由丢失问题
...
// Add路由，模拟数据与方法，可自行进行修改 test
// Add动态路由，`{ path: '*', redirect: '/404' }` 防止页面刷新，静态路由丢失问题

export function qiyeUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuQiye()
		.then(async (res) => {
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'qiye') qiyeUser(router, to, next);
